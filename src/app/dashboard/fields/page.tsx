...
import { PageHeader, Card, Badge, HealthDot, EmptyState, HelpNote } from "@/components/ui/primitives";
...
export default function FieldsPage() {
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setAdd(true)} className="btn-primary"><Plus className="h-4 w-4" /> {lang === "sq" ? "Fushë e Re" : "New Field"}</button>
          </div>
        }
      />
      ...
      <HelpNote>
        {lang === "sq"
          ? "Këtu menaxhon parcelat e tokës sate. Kliko “Fushë e Re”, vizato kufijtë në hartën satelitore dhe sipërfaqja llogaritet automatikisht. Kliko një fushë për të parë detajet, aktivitetet dhe analizat e tokës."
          : "Manage your land parcels here. Click “New Field”, draw the boundary on the satellite map and the area is calculated automatically. Click a field to see its details, activities and soil tests."}
      </HelpNote>

      {FIELDS.length === 0 ? (
        <EmptyState
          icon="🗺️"
          title={lang === "sq" ? "Ende pa fusha" : "No fields yet"}
          hint={lang === "sq" ? "Shto fushën tënde të parë duke vizatuar kufijtë në hartë." : "Add your first field by drawing its boundary on the map."}
          action={<button onClick={() => setAdd(true)} className="btn-primary"><Plus className="h-4 w-4" /> {lang === "sq" ? "Shto Fushën e Parë" : "Add First Field"}</button>}
        />
      ) : view === "map" ? (
        <Card className="p-0"><MapView fields={FIELDS} className="h-[28rem]" /></Card>
      ) : (
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs text-brand-charcoal/55">{fmtHa(f.area_ha)} · {f.village || f.municipality}</p>
                    </div>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-1.5">
                    ...
                    <Badge>{SOIL_LABELS[f.soil_type]?.[lang] ?? f.soil_type}</Badge>
                    <Badge tone="info">{IRRIGATION_LABELS[f.irrigation_type]?.[lang] ?? f.irrigation_type}</Badge>
                  </div>
                  <div className="mt-3 flex items-center justify-between border-t border-line pt-3 text-xs text-brand-charcoal/50">
                    <span>{last ? `${lang === "sq" ? "Aktiviteti i fundit" : "Last activity"}: ${relativeSq(last.activity_date)}` : (lang === "sq" ? "Pa aktivitete" : "No activity")}</span>
      ...
      <AddFieldModal open={add} onClose={() => setAdd(false)} />
    </div>
  );
}
