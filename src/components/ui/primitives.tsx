  ...
  return (
    <div className="mb-4 flex items-start justify-between gap-3">
      <div>
        ...
        {subtitle && <p className="text-xs text-brand-charcoal/55">{subtitle}</p>}
      </div>
      {action}
    </div>
  );
...
export function Badge({ tone = "neutral", children, className, style }: {
  ...
  return <span className={cn("badge", sevStyle[tone] ?? sevStyle.neutral, className)} style={style}>{children}</span>;
}

export function HealthDot({ health }: { health: "good" | "warning" | "critical" }) {
  const c = health === "good" ? "bg-brand-green-light" : health === "warning" ? "bg-brand-amber" : "bg-red-500";
...
export function EmptyState({ title, hint, icon = "🌱", action }: {
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-card border border-dashed border-line bg-white/50 px-6 py-12 text-center">
      <div className="mb-3 grid h-14 w-14 place-items-center rounded-full bg-brand-lime/50 text-2xl">{icon}</div>
      <p className="font-semibold text-brand-charcoal">{title}</p>
      {hint && <p className="mt-1 max-w-sm text-sm text-brand-charcoal/55">{hint}</p>}
